...
# UI Outline Head ----- 
ui <- navbarPage(title = "Causation in ecology", id = "inTabset", 
                 # Tab Panel 1 ####
                 {tabPanel("Introduction", id = "intro",
                 titlePanel("Informing the Use of Information Criteria"), 
                          mainPanel(img(src = "drought.jpg", height = 400, width = 600),
                                    p(" \"Pinang Tunggal Drought\"by Marufish is licensed under CC BY-SA 2.0", 
                                      style="font-size: 8pt"),
                                    
                                    p("Society increasingly demands accurate predictions of complex ecosystem processes
                                    under novel conditions to address environmental challenges. The process-level knowledge 
                                    required to do so is frequently misinformed through correlative model selection criteria, 
                                    such as Akaike's Information Criterion (AIC).
                                    These criteria are limited in their ability to select models that perform well 
                                    out of sample because they identify candidate models that are comparatively more 
                                    likely to reproduce observed outcomes. Causal understanding, however, is about 
                                    identifying parameter values, not outcomes. In this interactive application, we illustrate 
                                    via a concrete example that inference extending beyond model prediction, into the realm of causality, 
                                    can be badly misled by information-theoretic evidence.", style="font-size: 12pt")
                                    ),
                 fluidRow(column(1, offset = 6,
