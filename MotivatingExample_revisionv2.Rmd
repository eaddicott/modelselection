...
![DAG of Motivating Example](images/fig1newv2.jpg "Main Text Figure 1")
...
effort <- (sample.int(101, size = n, replace = TRUE) - 1) / 100
...
catch <- effort * (1+ sqrt(2 * logpopgrowth)) + 
         2 * sqrt(logpopgrowth) + nets / 5 + error.normal2
```
...
# ggsave("Addicott_Figure4.png", width = 10, height = 5)
```
...
beta1plot <- ggplot(df.md1, aes(x = obs, y = beta.1)) +
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0.4, 0.6)) +
  annotate("segment", x = 1010, xend = 1010, y = true.beta, yend = beta.1[960], colour = "red", size = 1, arrow = arrow(ends = "both", angle = 90, length = unit(.2, "cm"))) +
  annotate("text", label = "Bias", x = 960, y = (beta.1[960] + true.beta) / 2, angle = 90, size = 5)
...
beta2plot <- ggplot(df.md1, aes(x = obs, y = gamma.1)) +
  ...
  annotate("segment", x = 0, xend = 1010, y = -0.08, yend = -0.08, linetype = "dashed") +
  scale_x_continuous(expand = c(0, 0), limits = c(0, 1100)) +
  scale_y_continuous(expand = c(0, 0), limits = c(-.1, 0), breaks = c(-0.1, -0.08, -0.06, -0.04, -0.02, 0)) +
  annotate("segment",
    x = 1010, xend = 1010, y = true.gamma, yend = gamma.1[960],
    colour = "red", size = 1, arrow = arrow(ends = "both", angle = 90, length = unit(.2, "cm"))) +
  annotate("text", label = "Bias", size = 5, x = 960, y = (gamma.1[960] + true.gamma) / 2, angle = 90)

cowplot::plot_grid(beta1plot, beta2plot, labels = c("a", "b"))
# ggsave("Addicott_Figure2.png", width = 10, height = 5)
```
...
ggplot(df1, aes(x = Predicted, y = Actual - Predicted)) +
  ...
  xlab("Fitted") +
  ylab("Residuals")
# ggsave("Addicott_Figure3.png", width = 10, height = 5)
```
...
df.md2 <- data.frame(cbind(obs, beta.2))

ggplot(df.md2, aes(x = obs, y = beta.2)) +
  geom_line() +
  annotate("segment", x = 0, xend = 1010, y = true.beta, yend = true.beta, linetype = "dashed") +
  theme_classic() +
  theme(text = element_text(size = 17)) +
...
ggplot(df3) +
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0.4, 0.6)) +
  theme(text = element_text(size = 18))
# ggsave("Addicott_Figure5.png", width = 10, height = 5)
```
